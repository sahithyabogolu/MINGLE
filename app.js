/**
 * MINGLE - Complete Fixed Application Logic
 * Fixed: Robust PeerJS Mesh Connection + Google Public STUN Servers
 */
...
(function () {
    ...
    // PEERJS CONFIG WITH RELIABLE PUBLIC STUN SERVERS
    const PEER_CONFIG = {
        debug: 1,
        config: {
            iceServers: [
    ...
    async function handleCreateRoom() {
        ...
        try {
            ...
            showLobbyStatus('Room code conflict. Please try again.');
        }
    }
    ...
    async function handleJoinRoom() {
        ...
        try {
            ...
            const timeout = setTimeout(() => {
                conn.close();
                resetToLobby();
                showToast('Host unreachable. Check room code.');
            }, 12000);
    ...
    function approveGuest(peerId) {
        ...
        // Send host approval to joining user
        req.conn.send({
            type: 'JOIN_APPROVED',
        ...
        // Connect directly to existing peers in mesh network
        const existingPeers = Object.keys(state.participants).filter(id => id !== state.peerId && id !== peerId);

        // Broadcast new participant to all existing members
        broadcast({
        ...
        // If local stream active, trigger call to new peer
